...
class SpecialPendingReviews extends SpecialPage {
	...
	public function execute( $parser = null ) {
		...
		$out->addModules( 'ext.watchanalytics.pendingreviews.scripts' );
	...
	public function getClearNotificationTitle() {
		...
		$clearNotifyNS = $this->getRequest()->getVal( 'clearNotificationNS' ) ?? NS_MAIN;
		return Title::newFromText( $clearNotifyTitle, $clearNotifyNS );
	}
	...
	public function getStandardChangeRow( PendingReview $item, $rowCount ) {
		...
		if ( $item->title->isRedirect() ) {
			...
			if ( count( $item->newRevisions ) ) {
				...
				$previousViewedChange = $revisionStore->getPreviousRevision( $currentRevision );
